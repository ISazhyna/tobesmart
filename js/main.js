...
//#1 swap word to img and vice verse
$('.replace-image').click(function (e) {
    ...
    if (e.target && e.target.getAttribute("changed")=="false"){
        let str = fullQuery.innerHTML;
        let regexOb = new RegExp('\\b'+e.target.alt+'(|s)'+'\\b',"gi"); 
        let res = str.replace(regexOb, '<img class="replaced" alt="'+e.target.alt+'" src="images/replace/'+e.target.alt+'.png">');
        ...
        let str =fullQuery.innerHTML;
        let path='<img class="replaced" alt="'+e.target.alt+'" src="images/replace/'+e.target.alt+'.png">';
        let regexOb = new RegExp(path,"gi"); 
        let res = str.replace(regexOb, e.target.alt);
       fullQuery.innerHTML = res;
    }
});
...
function onDragStart(event) {
    ...
    addTargetId(document.querySelector("."+target.id));
}
...
//overlay on clicking over yellow or red
function on(color) {
    console.log(color);
